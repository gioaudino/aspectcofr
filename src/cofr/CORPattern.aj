...
public abstract aspect CORPattern {
	...
	abstract pointcut creation(Object caller);
	...
	Response around(Object caller) throws InstantiationException, IllegalAccessException, ClassNotFoundException: creation(caller){
		...
		System.out.println("Request was handled by " + response.getHandler().getName() + ". Response status is "
				+ response.getParams().get("status"));
		return response;
	}
}
